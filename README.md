...
3. **信号输出** — 对最新交易日截面打分，输出 Top 20 选股信号至 `signals/` 目录
...
| `signals/{timestamp}/signals_all.csv` | 全部股票信号排名（含 direction、market_trend） |

运行后终端会直接打印当日 Top 20 选股结果：
...
**输入特征（6 维）：** OPEN、CLOSE、HIGH、LOW、VOL、VWAP（由 `FeatureEngineer` 从原始 OHLCV 数据提取）
...
| 回测指标 | Sortino Ratio + IR | 含回撤惩罚 + 换手惩罚 + IR 评价 |
| 佣金 | 万 2.5 | 双边 |
...
| 训练步数 | 100000 步 | REINFORCE 策略梯度 + QFR 奖励塑形 |
| 公式最大长度 | 16 tokens | |
| Entropy 系数 | 0.1 → 0.05 | 线性退火，防止策略坍塌 |
| IC 奖励权重 | 5.0 | 与 Sortino 量级对齐 |
| QFR 参数 | λ=0.02, α=100, η=7.5e-4, δ=0.3 | IR 门槛动态提升 |
...
├── model_core/            # 核心模块
│   ├── config.py          # A股参数配置（佣金、印花税、涨跌停、T+1、股票池过滤、QFR、Entropy等）
│   ├── data_loader.py     # AshareDataLoader：从 CSV 构建特征张量 + 股票池过滤（板块/ST/次新股）
│   ├── factors.py         # 特征工程（6维 OHLCV 特征 + 技术指标因子）
│   ├── ops.py             # 数学算子（ADD, SUB, GATE, DECAY 等）
│   ├── vm.py              # 栈式虚拟机，执行公式 token 序列
│   ├── model.py           # 核心 Transformer 模型 + MTP Head
│   ├── backtest.py        # AshareBacktest：截面回测引擎（Sortino + IR 评分）
│   ├── engine.py          # AlphaEngine：RL 训练循环（QFR 奖励塑形 + Entropy 退火）
│   ├── report.py          # StrategyReport：OOS 业绩评估 + 可视化
│   └── signal_writer.py   # 信号输出（CSV 格式，含市场趋势过滤）
├── data/                  # 本地数据存储
